...
class AStarPlanner(Node):
    def __init__(self):
        ...
        # Диагностический статус планировщика
        ...
        self.get_logger().info("Статус планировщика: /path_planner/status")
    ...
    def get_path_callback(self, request, response):
        ...
        if current_map is None:
            ...
            self.publish_status("MAP_NOT_RECEIVED")
        ...
        try:
            ...
            self.get_logger().error(f"Ошибка TF преобразования цели: {e}")
            self.publish_status("TF_GOAL_ERROR")
        ...
        try:
            ...
            self.publish_status("TF_ROBOT_ERROR")
        ...
        if not self.is_valid(start_grid, current_map):
            ...
            if start_grid is None:
                ...
                self.publish_status("START_BLOCKED")
        ...
        if not self.is_valid(goal_grid, current_map):
            ...
            self.publish_status("GOAL_BLOCKED")
        ...
        if not raw_path_grid:
            ...
            self.publish_status("PATH_NOT_FOUND")

            response.success = False
            response.path = Path()
            return response

        # 2. Line-of-Sight упрощение
        ...
        # Сохраняем ориентации для визуализации
        all_orientations = []
        ...
        self.publish_status("PATH_READY")
    ...
    def publish_orientations_markers(self, frame_id, orientations):
        ...
        # Сначала очищаем предыдущие маркеры
        ...
        self.get_logger().debug(
            f"Опубликовано {len(marker_array.markers) - 1} маркеров ориентации"
        )
            ...
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    ...
                    if nxt not in visited:
                        ...
                        if 0 <= nxt[0] < map_msg.info.width and \
                           0 <= nxt[1] < map_msg.info.height:
                            queue.append(nxt)
